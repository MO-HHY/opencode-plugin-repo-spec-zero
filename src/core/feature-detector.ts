...
import { DetectedFeatures, RepoType, RepoStructure, FEATURE_FLAGS } from '../types';
...
export class FeatureDetector {
    ...
    private async detectFromPackageFiles(
        ...
        if (fs.existsSync(pkgPath)) {
            try {
                ...
                if (allDeps['@nestjs/core']) frameworks.add('nest');
                if (allDeps['react']) { frameworks.add('react'); features.add(FEATURE_FLAGS.HAS_REACT); }
                if (allDeps['vue']) { frameworks.add('vue'); features.add(FEATURE_FLAGS.HAS_VUE); }
                if (allDeps['@angular/core']) { frameworks.add('angular'); features.add(FEATURE_FLAGS.HAS_ANGULAR); }
                ...
                // Quality
                if (allDeps['typescript']) features.add(FEATURE_FLAGS.HAS_TYPES);
                if (allDeps['eslint'] || allDeps['biome']) features.add(FEATURE_FLAGS.HAS_LINTING);
     ...
     */
    private async detectStructure(repoPath: string): Promise<RepoStructure> {
    ...
    private async detectLanguages(
        ...
        const scanDir = (dir: string, depth: number = 0): void => {
            if (depth > 3) return; // Max depth for performance
    ...
    private async detectPatterns(
        ...
        const checkFileContent = (filePath: string, patterns: Array<[RegExp, string]>): void => {
            try {
        ...
        // API patterns
        const apiPatterns: Array<[RegExp, string]> = [
        ...
        const scanSrcPatterns = (dir: string, depth: number = 0): void => {
            if (depth > 2) return;
    ...
    private determineRepoType(
        ...
        if (frameworks.has('react-native') || frameworks.has('flutter') || 
            frameworks.has('expo')) return 'mobile';
        
        // 3. Check for CLI (no frontend, no backend structure, has bin)
        if (features.has('has_bin') && !structure.hasFrontend && !structure.hasBackend) return 'cli';
        ...
        // 6. Backend-only
        if (structure.hasBackend || hasApi || hasDb || hasBackendFramework) return 'backend';
    ...
    private async detectPackageManager(
        repoPath: string
    ): Promise<DetectedFeatures['packageManager']> {
     ...
     */
    private async findEntryPoints(repoPath: string): Promise<string[]> {
        const candidates = [
