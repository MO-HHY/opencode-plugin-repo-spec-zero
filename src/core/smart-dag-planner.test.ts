...
import { DetectedFeatures, FEATURE_FLAGS, RepoType } from '../types.js';
...
describe('SmartDAGPlanner', () => {
    ...
    let mockDetector: FeatureDetector;
    
    const createMockFeatures = (overrides: Partial<DetectedFeatures> = {}): DetectedFeatures => ({
        repoType: 'backend' as RepoType,
    ...
    describe('shouldSkipAgent', () => {
        it('should skip optional agent when dependency failed', () => {
            ...
            const result = planner.shouldSkipAgent(
                agent,
                features,
                new Set(['other']),
                new Set(['failed-dep'])
