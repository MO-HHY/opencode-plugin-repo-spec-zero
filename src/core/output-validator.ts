...
import type { ValidationResult, SpecOSFrontmatter } from '../types.js';
import { parseFrontmatter } from '../prompts/output-schema.js';

export interface ValidationOptions {
    strict?: boolean;         // Fail on warnings
    autoFix?: boolean;        // Attempt to fix issues
    projectSlug?: string;     // For UID validation
}

export interface DetailedValidationResult extends ValidationResult {
    fixed?: string;           // Auto-fixed content if autoFix enabled
    fixedCount?: number;      // Number of fixes applied
}
...
export class OutputValidator {
     ...
     * Validate UID format
     */
    private validateUID(content: string): ValidationResult {
        const errors: string[] = [];
        const warnings: string[] = [];
     ...
     */
    private validatePromptVersion(content: string): ValidationResult {
        const errors: string[] = [];
        const warnings: string[] = [];
     ...
     */
    private validateEdges(content: string): ValidationResult {
        const errors: string[] = [];
        const warnings: string[] = [];
     ...
     */
    private validateCitations(content: string): ValidationResult {
        const warnings: string[] = [];
 ...
 */
export function validateAndFix(content: string, options?: ValidationOptions): DetailedValidationResult {
