 ...
 */
export function generateFrontmatter(data: SpecOSFrontmatter): string {
 ...
 */
export function parseFrontmatter(content: string): SpecOSFrontmatter | null {
    const match = content.match(/^---\n([\s\S]*?)\n---/);
    if (!match) return null;

    const yaml = match[1];
    const result: Partial<SpecOSFrontmatter> = {};
    ...
    for (const line of lines) {
        ...
        if (keyMatch) {
            if (inArray && currentKey) {
                (result as any)[currentKey] = arrayValues;
                arrayValues = [];
            }
            ...
            if (value === '' || value === undefined) {
                ...
                inArray = false;
                (result as any)[currentKey] = value.replace(/^["']|["']$/g, '');
            }
    ...
    return result as SpecOSFrontmatter;
}
