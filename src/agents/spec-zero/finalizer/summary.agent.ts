...
import type { AgentContext, AgentResult, PromptVersion } from '../../../types.js';
...
export class SummaryAgent extends SubAgent {
    ...
    async process(context: AgentContext): Promise<AgentResult> {
        ...
        try {
            ...
            const result = await nativeLLM.execute<string>({
     ...
     */
    private extractSummaryFromOutput(content: string, maxLength = 500): string {
    ...
    private buildSummaryPrompt(
        ...
        if (sharedContext) {
            ...
            prompt += `\n## Execution Metadata\n`;
            prompt += `- Agents executed: ${(metadata as any).agentsExecuted?.length || 0}\n`;
            prompt += `- Total duration: ${Math.round(((metadata as any).durationMs || 0) / 1000)}s\n`;
            prompt += `- Key files analyzed: ${(metadata as any).keyFilesLoaded?.length || 0}\n`;
        }
...
- **Analysis Date:** ${(metadata as any).analysisDate}
- **Duration:** ${Math.round(((metadata as any).durationMs || 0) / 1000)}s
- **Agents Executed:** ${(metadata as any).agentsExecuted?.length || 0}
- **Key Files Loaded:** ${(metadata as any).keyFilesLoaded?.length || 0}
`;
        }
     ...
     */
    private generateIndex(projectSlug: string, sharedContext: SharedContext | undefined): string {
        const today = new Date().toISOString().split('T')[0];
        const executedAgents = sharedContext?.getExecutedAgentIds() || [];
...
`;
     ...
     */
    private wrapDiagramWithFrontmatter(content: string, type: string, projectSlug: string): string {
        const today = new Date().toISOString().split('T')[0];
        return `---
