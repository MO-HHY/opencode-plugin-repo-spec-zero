...
import type { AgentContext, AgentResult, PromptVersion, SpecsManifest, AnalysisEntry } from '../../../types.js';
    ...
    async process(context: AgentContext): Promise<AgentResult> {
        ...
        try {
            ...
            if (!manifest) {
                ...
                manifest = manager.createInitialManifest(
                    projectSlug,
                    repoUrl,
                    specsConfig?.remoteUrl || '',
                    pluginVersion
                );
            }
...
`;
     ...
     */
    private buildManualSection(projectSlug: string): string {
...
- Issue Tracker: *Add link*`;
     ...
     */
    private formatRepoType(repoType: string): string {
     ...
     */
    private formatAgentTitle(agentId: string): string {
     ...
     */
    private buildSummary(filesWritten: string[], projectSlug: string): string {
