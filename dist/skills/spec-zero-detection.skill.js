...
export class SpecZeroDetectionSkill {
    ...
    checkFiles(fileList, patterns) {
        let matches = 0;
        for (const pattern of patterns) {
            // specific file match
            if (!pattern.includes('*')) {
                if (fileList.includes(pattern))
                    matches++;
            }
            else {
                // Wildcard match (simplified)
                const regex = new RegExp('^' + pattern.replace(/\./g, '\\.').replace(/\*/g, '.*') + '$');
                if (fileList.some(f => regex.test(path.basename(f))))
                    matches++;
            }
        }
        return matches > 0 ? 1 : 0; // Binary score for now, can be improved
    }
    checkDirectories(fileList, patterns) {
        let matches = 0;
        for (const pattern of patterns) {
            if (fileList.some(f => f.includes(pattern)))
                matches++;
        }
        return matches > 0 ? 1 : 0;
    }
    async checkDependencies(repoPath, dependencies) {
        let matches = 0;
        // Check package.json
...
//# sourceMappingURL=spec-zero-detection.skill.js.map