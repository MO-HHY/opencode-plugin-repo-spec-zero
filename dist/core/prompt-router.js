...
export class PromptRouter {
    registry;
    promptsDir;
    constructor(registry, promptsDir) {
    ...
    route(context) {
        ...
        const previousContext = this.buildPreviousContext(context.previousOutputs, definition.dependsOn);
        // 5. Prepare diagram instructions
        const diagramInstructions = this.buildDiagramInstructions(definition.diagrams, definition.id);
        // 6. Compose final prompt
        return {
            systemPrompt: this.composeSystemPrompt(baseSystem, context.repoType),
            analysisPrompt: this.composeAnalysisPrompt(loaded.content, previousContext, diagramInstructions),
            outputSchema,
            diagramInstructions,
    ...
    selectPrompt(context) {
        const applicable = this.registry.findApplicable(context.repoType, context.detectedFeatures, context.completedAgents);
        // Filter for current agent (match on produces)
        const forAgent = applicable.filter(p => p.produces.some(prod => prod.includes(context.currentAgentId)));
        if (forAgent.length === 0) {
            throw new Error(`No prompt found for agent ${context.currentAgentId} ` +
     ...
     */
    buildPreviousContext(outputs, dependencies) {
        if (dependencies.length === 0 || dependencies[0] === '*')
            return '';
        const sections = ['## Context from Previous Analyses\n'];
     ...
     */
    buildDiagramInstructions(types, promptId) {
     ...
     */
    composeSystemPrompt(base, repoType) {
        return `${base}
...
`;
     ...
     */
    composeAnalysisPrompt(specific, previousContext, diagrams) {
        let prompt = '';
        if (previousContext) {
 ...
 * Factory function
 */
export function createPromptRouter(registry, promptsDir) {
