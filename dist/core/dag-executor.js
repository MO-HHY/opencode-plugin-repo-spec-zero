 ...
 */
export function selectDAG(mode) {
    switch (mode) {
        case 'generation':
...
export class DAGExecutor {
    ...
    getLayers() {
        ...
        while (remaining.size > 0) {
            ...
            for (const node of this.dag.nodes) {
                if (executed.has(node.agentId))
                    continue;
                if (!remaining.has(node.agentId))
                    continue;
     ...
     */
    async executeAgent(agentId, client) {
 ...
 */
export function createCustomDAG(agentIds, baseDag = DEFAULT_DAG) {
