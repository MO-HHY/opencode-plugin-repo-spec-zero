...
export class AuditReportAgent extends SubAgent {
    ...
    description = 'Compares fresh analysis with existing specs and generates AUDIT_REPORT.md.';
    systemPrompt = 'You are the audit agent that detects changes between analyses.';
    triggers = [];
        ...
        try {
            ...
            return {
                ...
                data: {
                    ...
                    proposedVersion,
                    promptVersion: { id: 'audit_report', version: '2', hash: 'native' },
                },
     ...
     */
    compareSpecs(existingSpecs, freshAnalysis, logger) {
        const changes = [];
        // Build reverse map: filename -> agentId
        const filenameToAgent = {};
     ...
     */
    compareContent(existing, fresh, filename) {
        const added = [];
        const removed = [];
        const modified = [];
     ...
     */
    generateReportMarkdown(report, projectSlug) {
...
`;
     ...
     */
    buildSummaryText(summary, currentVersion, proposedVersion) {
...
//# sourceMappingURL=audit-report.agent.js.map