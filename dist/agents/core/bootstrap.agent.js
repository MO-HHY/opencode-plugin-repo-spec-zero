...
import { DEFAULT_KEY_FILES } from '../../types.js';
import * as fs from 'fs';
import * as path from 'path';
export class BootstrapAgent extends SubAgent {
    id = 'bootstrap';
    name = 'Bootstrap Agent';
    description = 'Reads key repository files into shared context for downstream agents.';
    systemPrompt = 'You are a bootstrap agent that prepares context for analysis.';
    triggers = [];
    // Additional files to search for beyond the defaults
    static ADDITIONAL_KEY_FILES = [
        // Entry points
...
`.trim();
     ...
     */
    async loadGlobPattern(baseDir, fileDef, sharedContext) {
        const loaded = [];
     ...
     */
    async detectAdditionalFiles(baseDir, repoType, sharedContext) {
        const detected = [];
        // Detect schema files
