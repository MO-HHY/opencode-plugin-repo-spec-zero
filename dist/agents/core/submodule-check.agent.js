...
export class SubmoduleCheckAgent extends SubAgent {
    id = 'submodule_check';
    name = 'Submodule Check Agent';
    description = 'Checks and initializes the specs Git submodule, detecting operation mode.';
    systemPrompt = 'You are a submodule management agent that prepares the specs repository.';
    triggers = [];
    ...
    async process(context) {
        ...
        try {
            ...
            if (!state.exists) {
                ...
                const result = await this.createSubmodule(manager, baseDir, specsFolder, projectSlug, pluginVersion, options, logger);
                state = result.state;
                created = true;
                initialized = true;
            }
            ...
            if (sharedContext) {
                ...
                if (state.existingSpecs) {
                    for (const [filename, content] of state.existingSpecs) {
                        sharedContext.addKeyFile(`.spec-zero/existing/${filename}`, content, 15000 // Larger limit for spec files
                        );
                    }
                }
            }
     ...
     */
    async createSubmodule(manager, baseDir, specsFolder, projectSlug, pluginVersion, options, logger) {
        ...
        const specsRepoUrl = await manager.createSpecsRepo(projectSlug, githubOwner, options.privateRepo);
        // 4. Add as submodule
        ...
        const manifest = manager.createInitialManifest(projectSlug, repoUrl, specsRepoUrl, pluginVersion);
     ...
     */
    buildSummary(state, created, initialized, repoSha) {
        const lines = [
...
//# sourceMappingURL=submodule-check.agent.js.map